...
RSpec.describe Label, type: :system do
  ...
  describe '#edit' do
    let(:label_name) { 'labelA' }
    before { visit edit_label_path(label) }
    context 'with valid form' do
      before do
        fill_in 'label_name', with: label_name
      end
      it 'success to update the label' do
        click_button '保存'
        expect(current_path).to eq labels_path
        expect(page).to have_content "ラベル「#{label_name}」を更新しました。"
        expect(Label.find(label.id).name).to eq label_name
      end
    end
  end
