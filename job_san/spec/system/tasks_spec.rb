...
RSpec.describe Task, js: true, type: :system do
  ...
  let!(:login_user) { create(:user, password: user_password, password_confirmation: user_password) }
  before do
    travel_to(Time.zone.local(2020, 12, 24, 21, 0o0, 0o0))
    visit login_path
  ...
  describe '#index' do
    ...
    context 'when search tasks with a task_name' do
      ...
      before do
        fill_in 'タスク名', with: search_task_name[2..7]
        click_button '検索'
        sleep(0.3)
      end
